...
{
  double *shape,*quat;
  double fquat[4],conjqm[4],inertia[3],v_ang[3];
...
//      v[i][2] += dthlfm * f[i][2];
      ...
      v[i][0] += dthlfm * f[i][0] + dthlfm * (angmom[i][1]-angmom[i][2])*coe*pow(-1,i+time);
      v[i][1] += dthlfm * f[i][1] + dthlfm * (-angmom[i][2]+angmom[i][0])*coe*pow(-1,i+time);
      v[i][2] += dthlfm * f[i][2] + dthlfm * (-angmom[i][1]+angmom[i][0])*coe*pow(-1,i+time);
      ...
      vec3_to_vec4(quat,v_ang,f_cross);

      // update quaternion 4-momentum by 1/2 step
       conjqm[0] += dt * fquat[0] + dt*coe_*f_cross[0]*pow(-1,i+time+1);
       conjqm[1] += dt * fquat[1] + dt*coe_*f_cross[1]*pow(-1,i+time+1);
       conjqm[2] += dt * fquat[2] + dt*coe_*f_cross[2]*pow(-1,i+time+1);
       conjqm[3] += dt * fquat[3] + dt*coe_*f_cross[3]*pow(-1,i+time+1);
...
void FixNVEDot::final_integrate()
{
  ...
  double *quat;
  double fquat[4],conjqm[4],f_cross[4],v_ang[3];
